...
def influ_to_sql(update=True):
    ...
    """
    influ = pd.read_csv(f"{processed_data}\\influ.csv")
    influ = create_sql_dates(influ)
    ...
    if update is True:
        update_sql_table(influ, "influ", conn, primary_key)
        ...
        create_table(influ, "influ", conn, primary_key, foreign_key, ref_table, ref_col)
    ...
    open(
        f"{update_logs_folder}\\influ_{str(pd.to_datetime('today').date())}.txt", "a"
    ).close()
