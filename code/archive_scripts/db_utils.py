...
def check_for_new_addresses(address_df, current_addresses):
    address_df['pk'] = address_df['member_id'].astype(str) + address_df['address']
    current_addresses['pk'] = current_addresses['member_id'].astype(str) + current_addresses['address']

    new_addresses = address_df[-address_df.pk.isin(current_addresses.pk)].copy()
    new_addresses.to_csv('new_adds.csv')
    current_addresses['active'] = np.where(current_addresses.member_id.isin(new_addresses), 0,
                                          current_addresses['active'])
    new_addresses.drop_duplicates(inplace=True)
    new_addresses.reset_index(inplace=True)
    new_geocoded = geolocate_addresses(new_addresses, pd.read_csv('data/statewide.csv'))
    
    current_addresses.append(new_geocoded)
    current_addresses.to_csv('addresses.csv')
...
def get_csv_files():
    ...
    folder_dicts = [address_dict, incident_dict, utl_dict, vacc_dict]
    ...
    return tables, address_dict, incident_dict, utl_dict, vacc_dict
...
def clean_addresses(address_dict, tables, rename_dict, drop_cols, db_name, update=False, geocode_all=False, one_file=False):
    ...
    address_dfs = list(address_dict.keys())[1:]

    address_df = address_dict[address_dfs[1]].copy()

    for df in list(address_dict.keys())[2:]:
        address_df = address_df.append(address_dict[df])
    
    #drop, rename, and reformat col names
    address_df.drop(drop_cols, axis=1, inplace=True)
    address_df.rename(columns=rename_dict, inplace=True)

    address_df['as_of'] = pd.to_datetime('today').date()

    address_df['active'] = 1

    if update:
        q = 'SELECT * FROM addresses WHERE active=1'
        conn = sqlite3.connect(db_name)
        address_df = check_for_new_addresses(address_df, pd.read_sql(q, conn))
        conn.close()

    if geocode_all:
        address_df = geolocate_addresses(address_df)

    address_df.drop(['pk'], axis=1, inplace=True)

    return address_df

def clean_demos(tables):

    #Cognify codes all Hispanic/Latino ethnicity participants as unknown race
    #More usueful to code as Hispanic/Latino
...
def create_or_update_table(db_name, update_table=True):
    ...
    tables, address_dict, incident_dict, utl_dict, vacc_dict = get_csv_files()
    tables['addresses'] = clean_addresses(address_dict,tables, address_rename_dict, address_drop_cols, db_name, update=update_table)
    ...
    tables['inpatient'], tables['er_only'], tables['inpatient_snf'] = clean_utlization(utl_dict, inpatient_rename_dict, inpatient_drop,
                                                                                    ...
                                                                                    utl_drop_cols, update=update_table)
